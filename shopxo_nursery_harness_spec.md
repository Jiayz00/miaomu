...
在人工确认的开发环境中恢复完整源码，安装 PHP 8.0.2+ 与 Composer，准备测试数据库，验证安装/启动、管理员登录、首页、商品列表和详情。
...
部署环境、HTTPS、容量、媒体限制、隐私、通知渠道、备份恢复、性能预算和管理员角色全部由人工确认。Harness 只生成检查证据，不自动发布生产。
...
同一任务目录还包含 `workflow-history.json`、`requirement.md`、`impact-analysis.md`、`implementation-plan.md`、`test-plan.md`、`evidence.md`、`review.md` 和 `release-note.md`。其中 `task.json` 是机器授权合同，`workflow-history.json` 由 CLI 维护状态与审批审计，`requirement.md` 保存带来源的需求摘录和任务边界，其余 Markdown 分别承载分析、计划、真实证据、独立审查与发布/回滚说明。
...
计划批准事件先对 `requirement.md`、`impact-analysis.md`、`implementation-plan.md` 和 `test-plan.md` 计算制品 SHA-256，并计算任务关联的已解决需求决策上下文哈希。`preflight` 再把这些哈希与任务身份、业务目标/不变量、owner/reviewer/release_approver、需求、优先级、风险、范围、核心/数据声明、验收定义、测试、回滚和计划审批一起锁定到 `.harness/state/active-task.json`。任一授权字段、计划制品或关联决策变化都会使旧审批/活动状态失效，必须重新审查并运行 `preflight`。已有 active state 不能被再次 preflight 覆盖并改写 `scope_base_commit`；重新定基必须先按状态机返回分析阶段、修订计划并重新审批。生命周期状态、`evidence.md` 及合并/发布审批结果是受控后置内容，只能通过 Harness CLI 或指定证据文件更新。

任务文件和审批字段仍是仓库内容，哈希不能证明真实的人类独立审批。最终信任由 Git 平台审查、受保护分支和审查者身份提供。
...
| L2 | 普通服务、接口、展示功能 | 服务/API 测试、页面验证、人工审查 |
| L3 | 用户权限、询价状态、统计口径、价格历史、数据库变更、个人数据导出 | 授权/迁移/回滚/数据对账、独立审查 |
| L4 | 认证基础、框架核心、生产迁移、破坏性数据操作 | 专项设计、备份恢复演练；plan/merge 由 reviewer，release 必须由不同于 owner/reviewer 的第二身份审批；Harness 不自动执行生产动作 |
...
该 MCP 只读、不访问网络、不连接数据库、不存储凭据。GitHub、数据库或浏览器 MCP 只有在出现真实用例、权限模型和凭据方案后才按任务增加，不能为了“使用 MCP”而默认扩大权限。
...
- `task-approval`：记录 plan/merge/release 人工审批声明；plan/merge 匹配 reviewer，release 匹配独立 release_approver；
- `contract-hash`：输出授权字段哈希供人工核对；
- `task-check`：校验任务、需求编号、开放决策和风险字段；
- `plan-check`：校验影响分析、实施计划和测试计划；
- `preflight`：检查分支、状态、合同和路径并激活任务；
- `scope-check`：检查已跟踪、未跟踪、删除和重命名路径；
- `verify`：以 `shell=False` 运行任务声明的命令数组，代码硬限制单流输出不超过 1 MiB、单测试不超过 3600 秒，清理敏感环境、禁止网络/安装/发布/Git 变更命令，并记录真实退出码；测试前后业务指纹或 Harness/control-plane 哈希变化会使结果失败；
- `evidence-check`：校验证据完整性；
- `review-pack`：生成脱敏审查摘要；
- `release-check`：在任务已进入 `approved_for_merge` 或 `closed` 后复核合并/人工发布准备度，不执行发布；进入 `approved_for_merge` 前的同一套检查仅由 `task-transition` 内部调用。
...
先完成可运行基线和最小 P0 闭环，再逐步进入收藏、询价和统计。自动合并、自动生产发布、无人审批规则演进和复杂多代理编排不属于第一版。
