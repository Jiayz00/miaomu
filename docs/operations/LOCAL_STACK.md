...
- `/etc/miaomu/secrets/nursery_inquiry_hmac_key`；
...
询价防重复和手机号保护使用同一份实例 HMAC 密钥。`nursery_inquiry_hmac_key` 必须由仓库外脚本以 root:10001/0440 创建一次并在后续发布中保留，禁止轮换或写入 Git。app 入口只从 `/run/secrets/nursery_inquiry_hmac_key` 读取单行值，随后通过固定的 `PHP_NURSERY_INQUIRY_HMAC_KEY` FPM 环境项提供给 ThinkPHP；`clear_env = yes` 不得被删除，也不得用请求头、查询参数或 `PHP_VALUE`/`PHP_ADMIN_VALUE` 注入。
