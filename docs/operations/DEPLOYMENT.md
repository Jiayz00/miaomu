...
5. `/etc/miaomu/config/database.php` 与主模板哈希一致，恢复配置与独立恢复模板哈希一致；主/恢复应用 secret 和 `nursery_inquiry_hmac_key` 为 root:10001/0440，root DB secret 只能为 root:root/0400，均非空。HMAC 文件只在首次部署由 `deploy/prepare_runtime_secrets.py` 创建，后续发布必须保留，不得轮换。主/恢复 generated event 文件路径独立，bootstrap 前为 root:10001/0660，稳态为 root:10001/0440 且哈希进入 release manifest。不得读取或输出 secret 内容。
...
5. 运行镜像内 `/usr/local/lib/miaomu/environment_check.php`，验证 PHP/`intl`/`Normalizer`、release SHA、只读源码、有限写目录、应用 secret、HMAC secret 元数据、generated event、数据库 readiness 和 socket 元数据。入口脚本读取 HMAC 单行值并导出 `PHP_NURSERY_INQUIRY_HMAC_KEY`，FPM 通过固定 `env[...]` 传递；不允许删除 `clear_env = yes` 或改用请求参数覆盖。
