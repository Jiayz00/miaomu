...
- `deploy_contract`：`python tests/ops/test_deployment_contract.py`，检查两服务 Compose、PHP `intl`/Normalizer、schema-only bootstrap、HMAC secret、FPM socket、Caddy 只读挂载、端口 88、备份和回滚合同。
...
4. **数据库、迁移与权限边界**：确认数据库无宿主机端口，仅 internal backend；schema bootstrap 先证明空库并只建立表结构，不导入上游样例记录，写入非 1 号禁用管理员占位；运行 `initialize_nursery` 的 catalog/favorite/inquiry v1 前向迁移，核验五类表/索引和 `sxo_config` 台账、重复执行幂等及失败关闭。管理员登录凭据设置和真实后台会话作为后续运维动作，未设置时标记 blocked；同时检查安装器、SQL 控制台、在线升级、插件/主题上传、订单/支付路由不可达。
...
本任务只操作非生产测试数据，不读取生产数据库，不处理真实用户收藏/询价历史。数据库备份目录不在 Web root；secret 通过 `/etc/miaomu` 和 `/etc/miaomu-restore` 外部文件注入，应用只能读取所需 secret。初始化只创建禁用的非 1 号管理员占位，不设置登录凭据；后台认证和需要登录的管理员路由检查在本任务中保持 `blocked`，凭据由部署后人工运维动作设置。不得把占位账号或未执行的会话验证表述为通过。

## 未覆盖项

- 本地 Windows 工具链和 Docker daemon 的可用性不等同于服务器通过；服务器命令尚未执行前统一记 `not_run`。
- 远程 HTTP/浏览器、真实并发、MySQL 备份恢复、Caddy reload/recreate 和公网 :88 探测必须由 L4 broker 后续补证；空测试库的数据库/上传深备份与后台登录凭据设置在本次发布中记为 blocked/not_run。
