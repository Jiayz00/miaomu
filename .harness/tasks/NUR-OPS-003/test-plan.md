...
1. 服务拓扑：
   ...
   - app 与 db 只加入 internal backend且均无 ports；app 只在 `miaomu_fpm_socket` 创建 `/run/miaomu-fpm/php-fpm.sock`，group 10001、mode 0660。
2. Caddy 接入：
   - deploy/Caddyfile.miaomu 与 deploy/caddy-mounts.json 明确使用现有 Caddy v2.11.2、独立 127.0.0.1:88、root /var/www/html/public 和 FastCGI `unix//run/miaomu-fpm/php-fpm.sock`；公网 :88 不得监听。
   ...
   - 挂载合同只允许 Caddy 只读访问部署 public、uploads 与 `miaomu_fpm_socket`，并固定 supplemental group 10001；downloads、runtime、数据库、config 和 secret 均不得挂载。
3. 容器安全：
...
4. 配置与身份：
   ...
   - 主项目固定 miaomu；恢复项目需要显式独立 project、socket 卷、config 和 secret，恢复 socket 不挂 Caddy，命令不得硬编码 -p miaomu 覆盖恢复名称。
5. L4 入口：
   - Composer 严格检查运行在含 Composer 的 build/verify target，而非精简运行镜像。
   - 应用镜像包含真实环境检查入口，路径与 NUR-OPS-001 required_tests 一致。
   - PHP-FPM 包含只对 fpm-fcgi 生效的 prepend guard；规范化后的 SCRIPT_FILENAME 仅允许三个入口并拒绝 PATH_INFO，CLI 检查不被误拦截。guard 被标记为纵深防御，listener 安全来自 socket 卷与 group，而不是假设 PHP_VALUE/PHP_ADMIN_VALUE 无法覆盖。
...
6. 负变异：
   - 额外服务、任何 app/db ports、TCP FPM、socket mode 0666、缺少 group 10001、主/恢复 socket 共卷、root app、read_only=false 或 Docker socket 必须失败。
   - 移除 FPM guard、扩大 Caddy PHP matcher、把 HTTP 输入映射为 PHP_VALUE/PHP_ADMIN_VALUE、删除大小写/扩展/path-info/媒体拒绝或把 file_server 放到 deny 前必须失败。
...
3. app 构建、Composer strict/platform 检查、运行镜像无 Composer、容器环境检查和数据库 readiness；核验 socket 是 Unix socket、group 10001、mode 0660，且 app 与 `jia-caddy` 之外没有挂载者或 supplemental group。
4. 宿主机和容器均无 FPM TCP listener、MySQL 无宿主机端口；无卷容器与普通宿主机用户连接 socket 失败，Caddy 连接成功。发送名为 PHP_VALUE/PHP_ADMIN_VALUE 的 HTTP 头和查询参数，证明 Caddy 不把它们转换为 FastCGI 配置参数；再验证 FPM guard 的三个入口和 PATH_INFO 负例。
...
6. 保存原 Caddyfile、共享 Compose、docker inspect、挂载、supplemental groups、端口和 supervise 健康快照；验证完整候选配置。新增 public/uploads/socket 挂载或 group 10001 时只 recreate jia-caddy，只有三项挂载和 group 已存在且仅配置变化时才允许 reload。
...
恢复合同必须保证不同 project、数据卷、socket 卷、database.php 和 secrets；恢复 socket 卷默认不接入共享 Caddy。若需页面抽样，必须由 NUR-OPS-001 另行锁定临时 socket 挂载和仅回环入口，不能复用主 :88、主 socket 或主媒体卷。
...
- 本任务不访问 registry/服务器；候选 digest、Caddy 容器身份、目标架构摘要、镜像 ID、socket 卷/GID/userns/挂载与 88 端口状态未由本任务验证。
