...
- `AC-TASK-001`：离线合同测试确认 `config.json` 精确登记 15 个已核验 Hook，前后台导航、后台快捷菜单、商品按钮、首页赋值和用户中心替换视图均有对应过滤分支；默认快捷菜单 `178/364` 与 distribution/coupon/seckill 被拒，商品、站点和 nursery 正例保留。用户中心仅对外层空 view 与显式 default fallback 做精确替换，Windows 混合分隔符同样命中；插件模板的 6 个 `public/*` ModuleInclude、其他 module/plugin 路径、相似和大小写变体均保持不变，避免递归替换。用户中心替代视图保留资料、安全、消息、收藏和浏览历史。商品 list/slider 替代视图与固定上游模板的唯一语义差异是删除硬编码购物车节点，公开价格、单位、商品链接和三个商品模块 Hook 均保留；default direct 与显式 fallback 被替换，非 default 主题自有 direct 模板保持不变。真实页面结果留待 L4 运行环境验收。
...
VERIFY_CONTRACT_SHA256: 70af1c48e0eacc9525dbce2847805f5300f9855655447e82bf20dceb92099acb
...
结果：23 项合同测试通过；覆盖逐 Hook、逐拒绝控制器、23+4 直达拒绝、8 个仅隐藏入口、重复插件 URL 全量扫描、后台快捷菜单、Git 跟踪、导航、按钮、用户中心外层/嵌套 MyView、default/fallback/custom-theme 路径边界和商品模板受控差异的临时副本负变异。Harness 记录于 `.harness/runs/NUR-FEAT-001/20260713T165229053992Z-verify/`。

TEST_COMMAND: harness_selftest ["python", "scripts/harness_selftest.py"]
TEST_RESULT: harness_selftest exit_code=0

结果：60 项 Harness 自测通过；2 项需要 Windows 符号链接权限的用例明确 skip，不影响本任务业务断言。`source-check` 与 `scope-check NUR-FEAT-001` 另行执行并均为 exit code 0，范围检查为 tracked=6、untracked=0、无越界路径。
...
- Python 源码模型不能证明用户中心嵌套 `ModuleInclude()` 在真实 ThinkPHP/PHP 环境有限完成；服务器验收必须设置超时与输出上限，确认外层模板和公共模块只渲染一次且错误日志无递归或内存错误。
