...
|---|---|---|---|---|---|---|---|
| NUR-SEC-001 | d1825c5404054b535255d8fcad675a5dae0ab633 | app/service/GoodsService.php | `plugins_service_goods_field_status_update` runs after the update and cannot recover the old shelf state; a small previous_goods read/argument inside the existing transaction is required for truthful audit. | Low, isolated argument addition to an existing hook; upstream merge may need conflict review. | Remove the previous_goods read and hook field; nursery audit writes fail closed without changing ShopXO status behavior. | Codex-Review | pending |
