...
已有 active state 的任务不能再次 preflight 覆盖 `scope_base_commit`。首次实施前需要重新定基时，先按状态机返回 `ready_for_analysis`，修订计划并重新审批；已进入过 `implementing` 且只有计划制品变化的任务，可在安全状态执行受控 `state-recover` 后，以未变化的合同/策略/决策重新 `preflight`，不重复 plan 审批，但所有 warning 和差异必须进入独立 merge/release 审查。
