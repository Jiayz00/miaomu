...
function NurseryInquiryParseOptions($argv, $allowed)
{
    $options = [];
...
function NurseryInquiryOutput($result)
{
...
try {
    ...
    $options = NurseryInquiryParseOptions($argv, $action === 'migrate' ? ['actor', 'run-id'] : []);
    ...
    NurseryInquiryOutput(['msg'=>$e->getMessage(), 'code'=>-1, 'data'=>null]);
}
...
try {
    ...
    if($action === 'status')
    {
        $result = \app\plugins\nursery\service\InquiryMigration::Status();
    } elseif($action === 'preflight') {
        $result = \app\plugins\nursery\service\InquiryMigration::Preflight();
    } else {
        $result = \app\plugins\nursery\service\InquiryMigration::Run($options['actor'], $options['run-id']);
    }
} catch(\Throwable $e) {
    NurseryInquiryOutput(['msg'=>$e->getMessage(), 'code'=>-1, 'data'=>null]);
}
NurseryInquiryOutput($result);
?>
